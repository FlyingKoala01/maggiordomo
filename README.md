...
- **Skip duplicate downloads** (second switch in the sidebar, off by default): when a download starts and Chrome's download history already has a completed file with the same name (ignoring a ` (n)` suffix) and the exact same byte size that still exists on disk, the new download is cancelled and a notification says so, with *Open file* and *Show in folder* buttons; clicking the notification body downloads it anyway. A file with the same name but different content has a different size and downloads normally as `name (1).ext`. Extensions cannot read file contents, so this is a name-and-size fingerprint, not a hash. It needs the `downloads`, `downloads.open` and `notifications` permissions, requested when the switch is turned on. Downloads with unknown size (no `Content-Length`) are never skipped.
